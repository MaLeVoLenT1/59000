...
#ifdef Q_OS_WIN
    setPortName("/dev/ttyS2");//("COM1");

#elif defined(_TTY_IRIX_)
    setPortName("/dev/ttyS2");//("/dev/ttyf1");

#elif defined(_TTY_HPUX_)
    setPortName("/dev/ttyS2");//("/dev/tty1p0");

#elif defined(_TTY_SUN_)
    setPortName("/dev/ttyS2");//("/dev/ttya");

#elif defined(_TTY_DIGITAL_)
    setPortName("/dev/ttyS2");//("/dev/tty01");

#elif defined(_TTY_FREEBSD_)
    setPortName("/dev/ttyS2");//("/dev/ttyd1");

#elif defined(_TTY_OPENBSD_)
    setPortName("/dev/ttyS2");//("/dev/ttyS2");//("/dev/tty00");

#else
    setPortName("/dev/ttyS2");//("/dev/ttyUSB0");//("/dev/ttyS2");//
#endif
...
{
    lastErr = E_NO_ERROR;
    Settings.BaudRate=BAUD9600;//115200;
