/****************************************************************************
** Meta object code from reading C++ file 'output.h'
**
** Created: Fri Feb 28 12:07:40 2020
**      by: The Qt Meta Object Compiler version 62 (Qt 4.6.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../output.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'output.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.6.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_output[] = {

 // content:
       4,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
       8,    7,    7,    7, 0x0a,
      18,    7,    7,    7, 0x0a,
      32,    7,    7,    7, 0x0a,
      44,    7,    7,    7, 0x0a,
      60,   58,    7,    7, 0x0a,
      95,   88,    7,    7, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_output[] = {
    "output\0\0setup_1()\0oValvesCtrl()\0"
    "oGainCtrl()\0oreadSignal()\0e\0"
    "keyReleaseEvent(QKeyEvent*)\0Sample\0"
    "current4to20mAOutput(double)\0"
};

const QMetaObject output::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_output,
      qt_meta_data_output, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &output::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *output::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *output::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_output))
        return static_cast<void*>(const_cast< output*>(this));
    return QDialog::qt_metacast(_clname);
}

int output::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: setup_1(); break;
        case 1: oValvesCtrl(); break;
        case 2: oGainCtrl(); break;
        case 3: oreadSignal(); break;
        case 4: keyReleaseEvent((*reinterpret_cast< QKeyEvent*(*)>(_a[1]))); break;
        case 5: current4to20mAOutput((*reinterpret_cast< double(*)>(_a[1]))); break;
        default: ;
        }
        _id -= 6;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
